<template>
  <div>
    <el-form ref="bitForm" :model="bitForm" label-width="80px">
      <el-form-item>
        <el-tag>{{bitForm.record_time}}</el-tag>
      </el-form-item>
      ...
      <el-form-item label="内容：" prop="content" :colspan="12">
        <el-row>
          <el-col :span="12">
<!--            <el-input type="textarea" v-model="bitForm.content" rows="8"></el-input>-->
        ...
        <el-row>
          <el-col :span="12">
            <el-upload
              ...
              :file-list="bitForm.images"
              :auto-upload="false"
              >
              <el-button type="primary" icon="el-icon-picture">图片上传</el-button>
  ...
  export default {
    ...
    data() {
      return {
         ...
         */
        bitForm: {
          record_time: dateUtil.getCurrentTime('yyyy-mm-dd HH:mm'),      // 时间戳
          title: '',       // 标题
          // content: '',    // 内容
          richContent:'',   // 富文本内容
          images: [],   // 图片
        ...
        tags: [],
        editorOption:{},    // 编辑器配置项
        formData:new FormData()   // 表单对象：存储前端全部参数数据
      }
    },
    computed: {                         // 计算属性
      "val": function () {
    ...
    methods: {                            // 内部方法
      ...
      saveBit() {
        ...
        // 调用后端接口
        this.$api.recordBitApi.recordBit(this.formData).then(res=>{
